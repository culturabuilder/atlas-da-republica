...
def live_widgets(G, prefix):
    ...
    W = {}
    W["stats"] = f'<h4>O mapa hoje</h4><div class="row"><div><div class="big">{st["nodes"]}</div>órgãos e cargos</div><div><div class="big">{st["edges"]}</div>ligações com base legal</div><div><div class="big">{st["seats_filled"]}<small>de {st["seats_total"]}</small></div>cadeiras com ocupante conhecido</div></div><div class="src">Atualizado em {fmt_date(st.get("generated_at"))}</div>'
    # ministros
    ...
    W["ministros"] = f'<h4>Os ministros de Estado hoje · {len(ms)}</h4><div class="faces">{faces}</div><div class="src">Toque numa foto para abrir a pessoa na roda. Fonte: Planalto, conferido em página oficial.</div>'
    ...
    W["presidentes_casas"] = f'<h4>Quem preside as Casas</h4>{person(pc, "Presidente da Câmara · " + since(pc))}{person(ps, "Presidente do Senado · " + since(ps))}<div class="src">Fonte: Câmara e Senado, dados abertos.</div>'
    ...
    W["casas_composicao"] = f'<h4>Câmara, por partido</h4>{bars(N["br-deputado-federal"].get("people") or [], 513)}<h4 style="margin-top:12px">Senado, por partido</h4>{bars(N["br-senador"].get("people") or [], 81)}'
    # STF
    ...
    W["stf"] = f'<h4>O STF hoje</h4><div class="row"><div><div class="big">{len(ppl)}<small>de {stf.get("seats") or 11}</small></div>ministros{f" · {vac} vaga aberta" if vac == 1 else (f" · {vac} vagas abertas" if vac else "")}</div></div>' + (person(old, f"mais antigo · desde {fmt_date(old['started_at'])} · nomeado por {(old.get('entry') or {}).get('by_person') or 'Presidente'}") if old else "") + (person(new, f"mais recente · desde {fmt_date(new['started_at'])} · nomeado por {(new.get('entry') or {}).get('by_person') or 'Presidente'}") if new else "") + '<div class="src">Fonte: STF, composição atual.</div>'
    # TCU e PGR
    tcu = N.get("br-ministro-do-tcu") or {}; pgr = (N["br-procurador-geral-da-republica"].get("people") or [None])[0]
    W["tcu_pgr"] = f'<h4>Quem vigia, hoje</h4><div class="row"><div><div class="big">{len(tcu.get("people") or [])}<small>de {tcu.get("seats") or 9}</small></div>ministros do TCU</div></div>{person(pgr, "Procurador-Geral da República · " + since(pgr) + " · mandato de 2 anos")}<div class="src">Fonte: TCU e MPF, páginas oficiais.</div>'
    # sabatinas
    sabs = [c for c in G.get("changes", []) if c.get("kind") == "sabatina" and c.get("result") == "APROVADA_NO_PLENARIO"]
    last = sabs[0] if sabs else None
    W["sabatinas_ultima"] = f'<h4>Sabatinas desde 2023</h4><div class="row"><div><div class="big">{st.get("sabatinas", 0)}</div>indicações enviadas ao Senado</div><div><div class="big">{st.get("seats_sabatina", 0)}</div>cadeiras que passam por sabatina</div></div>' + (f'<div style="margin-top:8px">Última aprovada: <b>{esc(last.get("personName") or "")}</b> para <a href="{prefix}/#{last["positionId"]}">{esc(last.get("positionName") or "")}</a>, em {fmt_date(last["date"])}{", por " + str(last["votes"][0]) + " a " + str(last["votes"][1]) if last.get("votes") and last["votes"][0] is not None else ""}.</div>' if last else "") + '<div class="src">Fonte: Senado, mensagens (MSF) e votações.</div>'
    # distância
    W["distancia"] = f'<h4>Do seu voto ao presidente da Anatel</h4><ol class="path" style="margin-top:4px"><li>Você elege o <b>Presidente da República</b><small>CF/88 art. 77</small></li><li>O Presidente <b>indica</b> o presidente da Anatel<small>Lei 9.472/1997 art. 23</small></li><li>O Senado <b>sabatina e aprova</b> por maioria<small>CF/88 art. 52, III, f</small></li><li>Mandato de <b>5 anos</b>, sem demissão pelo Presidente<small>Lei 9.986/2000 arts. 6º e 9º</small></li></ol>'
    # orçamento
    ...
    W["orcamento_top"] = f'<h4>Empenhado em {year}, por ministério</h4><div class="bars">{rows}</div><div class="src">Total dos ministérios: {brl(total)} · inclui as entidades vinculadas · Portal da Transparência</div>'
    ...
    W["ultima"] = '<h4>Agora mesmo no Atlas</h4>' + (f'<div>Mudança mais recente: <b>{esc(chg.get("personName") or "")}</b>, {KIND.get(chg["kind"], chg["kind"])} em <a href="{prefix}/#{chg["positionId"]}">{esc(chg.get("positionName") or "")}</a>, {fmt_date(chg["date"])}.</div>' if chg else "") + (f'<div style="margin-top:6px">Notícia mais recente: <a href="{prefix}/#news:{nw["id"]}">{esc(nw["title"])}</a> <small style="font-family:var(--mono);color:var(--ink-3)">{esc(nw.get("publication") or "")} · {fmt_date(nw["date"])}</small></div>' if nw else "") + f'<div class="src">Fontes: DOU, Senado, páginas oficiais e {st.get("news_sources", 8)} veículos, lidos todo dia.</div>'
    # transição 2027
    ...
    W["transicao"] = f'<h4>Transição de 2027</h4><div class="row"><div><div class="big">{free + 2}</div>cadeiras que trocam com o novo Presidente</div><div><div class="big">{fixed}</div>mandatos fixos que atravessam a posse</div><div><div class="big">{life}</div>cargos vitalícios</div></div><div class="src">Contagem sobre os ocupantes conhecidos hoje. A aba Transição da roda acompanha cadeira a cadeira.</div>'
